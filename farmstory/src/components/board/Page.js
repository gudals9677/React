...
const Page = ({ serverData, cate1, cate2 }) => {
    ...
    return (
        <div class="page">
            {/* 이전 */}
            {serverData.prev && (
                <Link to={`/board/list?cate1=${cate1}&cate2=${cate2}&pg=${serverData.start - 1}`} className="prev">
                    이전
                </Link>
            )}
            ...
            {/* 다음 */}
            {serverData.next && (
                <Link to={`/board/list?cate1=${cate1}&cate2=${cate2}&pg=${serverData.end + 1}`} className="next">
                    다음
                </Link>
            )}
        </div>
